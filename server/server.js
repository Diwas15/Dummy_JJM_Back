...
app.use('/',function(req,res,next){
  ...
  if(req.path==='/'){
    ...
    app.use(express.static(path.join(__dirname,"../build"),{setHeaders:function(res,path,stat){res.set('Set-Cookie', `token=${tok};HttpOnly;same-site='none'`), res.set('Cache-Control','max-age=0, must-revalidate')}}));
